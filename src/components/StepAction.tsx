...
export default function StepAction({ className, current = 1, total = 6, ...rest }: StepActionProps) {
  const getCurrent = (current: number, total: number) => (current > total ? total : current);
  ...
  return (
    <div className={clsx(className, 'relative')} {...rest}>
      ...
      <div
        ...
        style={{
          width: `calc((100% - 24px) * ${((1.0 * getCurrent(current, total - 1)) / (total - 1)).toFixed(5)})`,
        }}
      />
        ...
        {_.range(1, total + 1).map((i) => (
          <div
            ...
            className={clsx(
              ...
              i <= getCurrent(current, total) ? 'bg-yellow border-white' : 'bg-white border-yellow',
            )}
          >
            {i}
          </div>
